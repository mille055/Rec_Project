...
# Download NLTK packages as they may not exist locally
...
def main(args):
    ...
    # Get recommendations based on keyword similarity
    results = get_recommendations(vocabulary, tfidf, tfidf_matrix, df_podcast, args.query, args.k)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        prog="Script to generate podcast recommendations based on keyword similarity between query and TF-IDF weighted document matrix",
        epilog="Example usage: keyword_similarity.py --query 'classical music' --k 10"
    )
    parser.add_argument("--query", default="", help="Keywords to find relevant podcasts. Defaults to empty string.")
    parser.add_argument("--k", type=int, default=10, help="Number of recommendations to return. Defaults to 10.")
    parser.add_argument("--file", default="keyword_similarity_recommendations.csv", help="CSV file name to save recommendation results as. Defaults to 'keyword_similarity_recommendations.csv'.")
